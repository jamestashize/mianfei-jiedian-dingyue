...
> 这个仓库作为每日免费节点订阅入口，统一指向 free.yunxiang.lol 的当天文章、导入教程和排错说明。
...
请打开上面的博客文章，在正文“免费订阅地址”区域获取当天可用入口。公开仓库只保存文章入口，真实订阅地址保留在当天正文，方便更新和失效说明。
...
本站每日北京时间 00:00 自动处理免费节点用户：重置流量、刷新订阅 Token、写入服务端动态数据，并同步 GitHub 分享仓库。适合先确认今日更新，再根据客户端复制订阅入口。 仓库里的 `sub/latest.txt` 只保存当天文章链接，不保存真实订阅地址。
